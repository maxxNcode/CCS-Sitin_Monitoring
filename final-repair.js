...
// The problem is that the utilities and extra closing divs are duplicated.
// We will find the navbar start, and find the Page Content comment.
// Everything between the utilities container start and Page Content should be reset.
const utilitiesMarker = '<div class="hidden lg:flex flex-none items-center gap-1 ml-auto">';
...
for (const file of filesToUpdate) {
    ...
    const utilIndex = html.indexOf(utilitiesMarker);
    const pageContentIndex = html.indexOf(pageContentMarker);

    if (utilIndex !== -1 && pageContentIndex !== -1 && utilIndex < pageContentIndex) {
        const head = html.substring(0, utilIndex);
        const tail = html.substring(pageContentIndex);
        
        // head contains the navbar start.
        // tail contains the Page Content comment and the rest.
        // We need to inject the standardizedNav and CLOSE the navbar div.
        
        // We also need to strip any trailing duplicate comments before utilIndex
        const cleanHead = head.replace(/(?:<!-- Desktop Menu utilities -->\s*)+$/, '');
        
        const newHtml = cleanHead + standardizedNav + '\n                </div>\n\n                ' + tail;
        
        fs.writeFileSync(filePath, newHtml, 'utf-8');
        console.log(`✅ Repaired and Standardized ${file}`);
    } else {
        console.log(`⚠️ Skipped ${file} - Markers not found or out of order`);
    }
}
