...
const pages = [
  { file: 'feedback-reports.html', activeLink: '/feedback-reports', activeLabel: 'Feedback' },
  { file: 'reservations.html', activeLink: '/reservations', activeLabel: 'Reservations' },
  { file: 'leaderboard.html', activeLink: '/admin/leaderboard', activeLabel: 'Leaderboard' },
  { file: 'manage-dropdowns.html', activeLink: '/admin/manage-dropdowns', activeLabel: 'Manage Dropdowns' },
  { file: 'analytics.html', activeLink: '/admin/analytics', activeLabel: 'Analytics' },
];
            ...
            <div class="drawer-side z-[200]">
                ...
                <ul id="sidebar-nav" class="menu p-3 min-h-full bg-base-100 text-base-content space-y-1">
                    <li class="sidebar-header flex flex-col items-center pt-5 pb-3 mb-1">
                        ...
                        <span class="sidebar-brand-text text-xs font-black tracking-tighter italic uppercase whitespace-nowrap leading-tight text-center">
...
for (const page of pages) {
  ...
  // 1. Fix stylesheet path (style.css -> ../style.css) only if not already ../
  ...
  // 5. Hide branding on desktop (add lg:hidden to flex-1 branding div)
  html = html.replace(
    /(<div class="flex-1 gap-3 flex items-center)(">)/,
    '$1 lg:hidden$2'
  );

  // 6. Fix asset paths (assets/ -> ../assets/ if not already)
  html = html.replace(/src="assets\//g, 'src="../assets/');

  // 7. Replace desktop menu div with toggle + utilities
  html = html.replace(
  ...
  // 8. Remove nav link buttons (Home, Students, Sit-in, etc.) - keep theme/notification/logout utilities
  // Remove blocks of <a href="/admin"...>Home</a> through Analytics links
  html = html.replace(
    /\s*<a href="\/admin" class="btn btn-ghost btn-sm">Home<\/a>[\s\S]*?(?=<div class="dropdown)/,
  ...
  // 10. Add toggle JS before </script> (after logout function)
  ...
  console.log(`✅ Applied sidebar to ${page.file}`);
}

console.log('\nDone! All 5 remaining pages updated.');
