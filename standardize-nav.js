...
const searchJS = `
        ...
        async function searchStudent() {
            ...
            if (!idNumber) return typeof showToast === 'function' ? showToast('Please enter an ID Number', 'warning') : alert('Please enter an ID Number');

            try {
                const res = await fetch(\`/api/admin/search-student/\${idNumber}\`);
                const data = await res.json();

                if (res.ok) {
                    document.getElementById('formIdNumber').value = idNumber;
                    document.getElementById('formStudentName').value = data.name;
                    document.getElementById('formSessions').innerText = data.sessionLeft;

                    document.getElementById('my_modal_1').close();
                    document.getElementById('my_modal_2').showModal();
                } else {
                    typeof showToast === 'function' ? showToast(data.error || 'Student not found', 'error') : alert(data.error || 'Student not found');
                }
            } catch (err) {
                console.error(err);
                typeof showToast === 'function' ? showToast('Search failed', 'error') : alert('Search failed');
            }
        }
        ...
        async function submitSitin() {
            const payload = {
                ...
                lab: document.getElementById('formLab').value,
                pcNumber: document.getElementById('formPcNumber').value
            };

            if (!payload.purpose || !payload.lab) {
                return typeof showToast === 'function' ? showToast('Please select a purpose and a lab', 'warning') : alert('Please select a purpose and a lab');
            }

            try {
                const res = await fetch('/api/admin/sit-in', {
                    method: 'POST',
                ...
                if (res.ok) {
                    typeof showToast === 'function' ? showToast('Sit-in recorded successfully!', 'success') : alert('Sit-in recorded successfully!');
                    document.getElementById('my_modal_2').close();
                    if (typeof fetchActiveSitins === 'function') fetchActiveSitins();
                    else if (typeof fetchStudents === 'function') fetchStudents();
                    else setTimeout(() => window.location.reload(), 1500);
                } else {
                    typeof showToast === 'function' ? showToast(data.error || 'Failed to record sit-in', 'error') : alert(data.error || 'Failed to record sit-in');
                }
            } catch (err) {
                console.error(err);
                typeof showToast === 'function' ? showToast('An error occurred', 'error') : alert('An error occurred');
            }
        }

        (function() {
            // Auto-fetch dropdown options for the Record Sit-in modal
            fetch('/api/dropdown-options')
                .then(r => r.json())
                .then(data => {
                    const purposeSelect = document.getElementById('formPurpose');
                    if (purposeSelect && data.purpose) {
                        purposeSelect.innerHTML = '<option value="" disabled selected>Select Purpose</option>';
                        data.purpose.forEach(opt => {
                            purposeSelect.innerHTML += \`<option value="\${opt.value}">\${opt.value}</option>\`;
                        });
                    }
                    const labSelect = document.getElementById('formLab');
                    if (labSelect && data.lab) {
                        labSelect.innerHTML = '<option value="" disabled selected>Select Lab Room</option>';
                        data.lab.forEach(opt => {
                            labSelect.innerHTML += \`<option value="\${opt.value}">\${opt.value}</option>\`;
                        });
                    }
                })
...
for (const file of filesToUpdate) {
    ...
    // Remove the old utilities block entirely.
    // In admin.html it was <!-- Global Utilities... -> </div>
    // In others it was <!-- Desktop Menu utilities... -> </div>
    
    // We match from the <div> containing 'ml-auto' to the end of the navbar
    const regex = /(<div class="hidden lg:flex flex-none items-center gap-1 ml-auto">)[\s\S]*?(?=(?:<!--[\s\S]*?-->\s*)?<main)/;
    
    html = html.replace(regex, (match, p1) => {
        // We close the navbar div that contains the utilities
        return standardizedNav + '\n                </div>\n\n                ';
    });

    // Clean up duplicate searchStudent/submitSitin if they exist
    // But since we don't know exactly what they look like, we only append if they are completely missing.
    // Actually, it's safer to only inject if `searchStudent()` is missing entirely.
    if (!html.includes('function searchStudent()')) {
        html = html.replace('</body>', '\\n<script>\\n' + searchJS + '\\n</script>\\n</body>');
    }

    if (html !== originalHtml) {
        fs.writeFileSync(filePath, html, 'utf-8');
        console.log(`✅ Standardized navigation utilities on ${file}`);
    } else {
        console.log(`⚠️ Skipped ${file} (No match found)`);
    }
}
