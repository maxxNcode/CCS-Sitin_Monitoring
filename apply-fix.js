...
// Find the start of the ml-auto div and match all the way up to `<main`
...
for (const file of filesToUpdate) {
    ...
    // Check if it already has duplicate search functionality that indicates corruption
    html = html.replace(regex, (match) => {
        return standardizedNav + '\n                </div>\n\n                ';
    });

    fs.writeFileSync(filePath, html, 'utf-8');
    console.log(`✅ Fixed DOM Structure in ${file}`);
}
